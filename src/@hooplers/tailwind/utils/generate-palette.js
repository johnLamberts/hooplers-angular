...
/**
 * Generates palettes fro the provided configuration
 * Accepts asingle color stirng or a Tailwind-like
 * color object. If provided Tailwind-color like object,
...
const generatePalette = (config) => {
  ...
  if(_.isString(config)) palette[500] = chroma.valid(config) ? config : null;
  ...
  // Generate a very dark and a very light version of the
  // default color to use them as boundary colors rather
  // than using pure white and pure black. This will stop
  // in between color's hue values to slipping into the grays.
  colors.unshift(
